...
# NVIDIA Management library for GPU performance check
try:
...
TRANSFORMERS_CACHE = Config.MODEL_CACHE_DIR
...
#    Returns:
#        :class:`~{full_output_type}`:`tuple(torch.FloatTensor)`:
...
def add_code_sample_docstrings(*docstr, tokenizer_class=None, checkpoint=None, output_type=None, config_class=None):
    def docstring_decorator(fn):
        ...
        if "SequenceClassification" in model_class:
            code_sample = TF_SEQUENCE_CLASSIFICATION_SAMPLE
        elif "QuestionAnswering" in model_class:
            code_sample = TF_QUESTION_ANSWERING_SAMPLE
        elif "TokenClassification" in model_class:
            code_sample = TF_TOKEN_CLASSIFICATION_SAMPLE
        elif "MultipleChoice" in model_class:
            code_sample = TF_MULTIPLE_CHOICE_SAMPLE
        elif "MaskedLM" in model_class:
            code_sample = TF_MASKED_LM_SAMPLE
        elif "LMHead" in model_class:
            code_sample = TF_CAUSAL_LM_SAMPLE
        elif "Model" in model_class:
            code_sample = TF_BASE_MODEL_SAMPLE
        else:
...
def is_tensor(x):
    """ Tests if ``x`` is a :obj:`tf.Tensor` or :obj:`np.ndarray`. """
    return isinstance(x, tf.Tensor) or isinstance(x, np.ndarray)
